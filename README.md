...
- 한 명이 한쪽 축의대에서 빠르게 입력하는 흐름에 맞춘 macOS 데스크톱 앱
- 봉투번호 자동 증가, 계좌이체 별도 차번, 이름, 모임, 관계, 대상, 금액, 성인/소인 식권, 입금방식, 메모 기록
- 한국 축의금에서 자주 쓰는 금액 빠른 선택, `0원`, `+1만원` 버튼
- 모임/관계/대상 직접 입력 시 이후 선택 목록에 반영
- 동명이인 즉시 감지, 기존 기록 확인 후 새 동명이인 저장
- 계좌 입금 기록은 실제 입금시간 직접 수정 가능
- 검색어 Enter 검색, 검색 초기화, 상태별 검색, 동명이인 빠른 필터, 기록 취소/복구/삭제
- 정산 현황, 평균 축의금, 봉투 누락, 성인/소인 식권 잔여량, 현금/계좌 합계 확인
- 마감 검수 순서 체크, 취소/복구 이력, 자동 백업, 백업 복원
- 전체내역, 검색용, 봉투 검수, 현금·계좌 정산, 식권 검수, 동명이인 점검, 고액·특이금액, 취소·삭제 이력을 포함한 `.xlsx` 추출
...
```bash
curl -fL --retry 8 --retry-delay 2 --retry-max-time 120 --retry-all-errors https://raw.githubusercontent.com/Rafe-Giho/wedding-ledger/main/scripts/install_macos.sh -o /tmp/install_wedding_ledger.sh
...
터미널 명령어를 쓰지 않으려면 [최신 릴리즈](https://github.com/Rafe-Giho/wedding-ledger/releases/latest)에서 직접 받을 수 있습니다.

1. 릴리즈 페이지의 `Assets`를 엽니다.
2. `wedding-ledger-버전-macOS.zip` 파일을 다운로드합니다.
3. zip을 압축 해제합니다.
4. `축의대 장부.app`을 `응용 프로그램` 또는 `~/Applications`로 옮깁니다.
5. 처음 실행이 막히면 Finder에서 앱을 Control-클릭한 뒤 `열기`를 선택합니다.

직접 다운로드 방식은 macOS Gatekeeper 격리 속성 때문에 `손상되었기 때문에 열 수 없습니다` 메시지가 뜰 수 있습니다. 이 경우 터미널 설치 방식을 사용하면 앱 설치 후 격리 속성을 제거하고 코드 서명 검증까지 처리합니다.
...
4. 설정에서 총 성인/소인 식권수, 예상 봉투수, 행사명, 운영 메모 입력
5. 테스트 데이터 초기화
6. 운영 모드로 전환
7. 봉투를 받으면 봉투 겉면에 번호를 적고 앱에 기록
8. 성인/소인 식권수와 입금방식, 메모까지 입력
...
- `전체내역`: 차번, 봉투번호, 계좌차번, 이름, 모임, 관계, 대상, 금액, 성인/소인 식권수, 방식, 상태, 입력시간, 메모
- `요약`: 총액, 평균, 입금방식별 합계, 식권/봉투 검수값
- `검색용`: 조건 입력, 사용법 안내, 결과 자동 표시
- `봉투 검수`: 봉투번호 누락, 중복, 취소/삭제 기록 확인
- `현금·계좌 정산`: 현금, 계좌, 기타 합계와 현금 실물 검수
- `식권 검수`: 성인/소인 준비 식권, 배부 식권, 남은 식권 확인
- `동명이인 점검`: 같은 이름의 정상 기록 목록
- `고액·특이금액`: 30만/50만/100만 이상과 빠른선택 외 금액 확인
- `취소·삭제 이력`: 잘못 입력한 기록 추적
